...
/**
 * A mix of :
 * 1. http://www.csee.umbc.edu/~squire/math/Complex.java and
 * 2. one function from http://introcs.cs.princeton.edu/java/97data/Complex.java.html
 * 3. Usecase dependent functions
 */
...
public strictfp class Complex extends Object
{
    ...
    public static Complex getRandomRoot(double a) {
        double rand = Math.floor(Math.random() * a);
        double realpart = Math.cos(2 * Math.PI * rand / a);
        double imagpart = Math.sin(2 * Math.PI * rand / a);
        return new Complex(realpart, imagpart);
    }


    public static Complex convertRootToComplex(double rootnum, double numroots) {
        double realpart = Math.cos(2 * Math.PI * rootnum / numroots);
        double imagpart = Math.sin(2 * Math.PI * rootnum / numroots);
        return new Complex(realpart, imagpart);
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(getRandomRoot(5.0).toString());
